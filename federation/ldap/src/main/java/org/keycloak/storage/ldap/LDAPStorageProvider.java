    ...
    public boolean updateCredential(RealmModel realm, UserModel user, CredentialInput input) {
    	if (editMode == UserStorageProvider.EditMode.READ_ONLY
                || (editMode == UserStorageProvider.EditMode.WRITABLE
                    && kerberosConfig.isAllowKerberosAuthentication()
                    && kerberosConfig.isUseKerberosForPasswordChange())) {

            if (!kerberosConfig.isAllowKerberosAuthentication() || !kerberosConfig.isUseKerberosForPasswordChange()) {
                throw new ReadOnlyException("Federated storage is not writable");
            }

            if (!CredentialModel.PASSWORD.equals(input.getType()) || !(input instanceof PasswordUserCredentialModel)) {
                return false;
            }
